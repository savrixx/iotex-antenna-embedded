...
void test_get_account() {
    ...
    for (int i = 0; accounts[i]; i++) {
        ...
        UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetAccount, accounts[i]), NULL);
        UNITTEST_ASSERT_EQ(url[strlen(url) - 1], i + '0');
    }

    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetAccount, TEST_ACCOUNT_ADDR), NULL);
...
void test_get_chainmeta() {
    ...
    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetChainMeta), NULL);
    UNITTEST_ASSERT_NE(url[strlen(url) - 1], '/');

    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetChainMeta, 1, 2, 3), NULL);
    UNITTEST_ASSERT_NE(url[strlen(url) - 1], '/');

    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetChainMeta, "nothing"), NULL);
    UNITTEST_ASSERT_NE(url[strlen(url) - 1], '/');
...
void test_get_action_by_hash() {
    ...
    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetActionsByHash, TEST_ACTION_HASH), NULL);
...
void test_get_action_by_addr() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetActionsByAddr, TEST_ACTION_ADDR, start, count), NULL);
...
void test_transfers_by_block() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetTransfersByBlock, i), NULL);
        UNITTEST_ASSERT_EQ(url[strlen(url) - 1], i + '0');
    }

    UNITTEST_ASSERT_NE(req_compose_url(url, sizeof(url), IotexReqGetTransfersByBlock, atoi(TEST_TRANSFERS_BLOCK)), NULL);
